const portfolio_stats = require("../portfolioStats/portfolioStats");
...
test(" fetching portfolio  ", () => {
  expect(portfolio_stats.fetchPortfolio(sample_portfolio)).not.toBeUndefined();
});

test(" total value > 0  ", async () => {
  const data = await portfolio_stats.portfolio_total_value(
    sample_price_fetch_data,
    sample_portfolio
  );
